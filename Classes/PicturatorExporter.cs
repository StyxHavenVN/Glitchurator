...
// Encapsulates the complete export state, layers, and configuration for slider generation
public sealed class PicturatorExportRequest : IDisposable
{
    ...
    // Captures only the visible ball path without rendering raster body scanlines (invisible-body export)
    public static PicturatorExportRequest CaptureBallOnly(SliderPicturatorVm vm)
    {
        ...
        if (ball == null)
        {
            throw new InvalidOperationException("Enable Sliderball and load at least one visible slider path.");
        }
        ...
        return new PicturatorExportRequest
        {
            ...
            Image = blank,
            BallSlider = ball,
            Shape = vm.SelectedSlider?.DeepCopy(),
            Start = ball.Pos,
            ImageStart = new Vector2(0, 0),
            Time = vm.TimeCode,
            Duration = vm.Duration,
            Resolution = vm.YResolution,
            Viewport = vm.ViewportSize,
            Quality = 1,
            MinimumTumourLength = 12,
            CompositeField = new double[,] { { 1.2 } },
            MotionGraph = vm.BallGraphEnabled ? vm.BallGraphPoints.ToArray() : null,
            MultiplexMotion = vm.CreateMultiplexMotion(0.1),
            SampleInterval = vm.ChainAllVisibleBallPaths ? Math.Min(1, vm.BallSwitchMilliseconds) : 1
        };
    }

    // Freezes active layers, shader fields, and motion parameters into an immutable export snapshot
    public static PicturatorExportRequest Capture(SliderPicturatorVm vm)
    {
        if (vm.Bm == null)
        {
            throw new InvalidOperationException("Import a shape slider or image first.");
        }
        ...
        if (layers.Length == 0)
        {
            throw new InvalidOperationException("Show at least one layer before exporting.");
        }
        ...
        var request = new PicturatorExportRequest
        {
            ...
            MotionGraph = vm.BallGraphEnabled ? vm.BallGraphPoints.ToArray() : null,
            Path = vm.BeatmapPath,
            Image = (Bitmap)vm.Bm.Clone(),
            BallSlider = ball,
            Shape = vm.SelectedSlider?.DeepCopy(),
            Start = ball?.Pos ?? new Vector2(vm.SliderStartX, vm.SliderStartY),
            ImageStart = new Vector2(vm.ImageStartX, vm.ImageStartY),
            Time = vm.TimeCode,
            Duration = vm.Duration,
            Resolution = vm.YResolution,
            Viewport = vm.ViewportSize,
        ...
        // If multiple layers exist, synthesize a unified composite scalar field
        if (layers.Length > 1 || layers[0] != vm)
        {
            ...
            double factor = (vm.YResolution - 16.0) / 480.0;
            ...
            {
                request.Dispose();
                throw new InvalidOperationException("Combined image is too large. Move the layers closer or reduce their size.");
            }
            ...
            {
                double radius = layer.NativeSliderShading 
                    ? Beatmap.GetHitObjectRadius(layer.TargetCS) * (layer.YResolution - 16.0) / 480.0 
                    : 0;

                using var glitch = layer.CaptureNativeGlitch();
                double[,] source = null;

                var recolor = Tools.SlideratorStuff.SliderPicturator.Recolor(
                    layer.Bm, 
                    Color.White, 
                    Color.White, 
                    Color.Black,
                    ...
                    layer.Quality, 
                    radius, 
                    glitch?.Build(radius, layer.Quality), 
                    f => source = f);

                recolor.Item1.Dispose();

                var rect = layer.PreviewImageBounds;
                int left = (int)Math.Round((rect.X - bounds.X) * factor);
                int top = (int)Math.Round((rect.Y - bounds.Y) * factor);
                int w = Math.Max(1, (int)Math.Round(rect.Width * factor));
                int h = Math.Max(1, (int)Math.Round(rect.Height * factor));

                for (int y = 0; y < h && top + y < height; y++)
                {
                    for (int x = 0; x < w && left + x < width; x++)
                    {
                        int srcX = Math.Min(source.GetLength(0) - 1, x * source.GetLength(0) / w);
                        int srcY = Math.Min(source.GetLength(1) - 1, y * source.GetLength(1) / h);
                        double value = source[srcX, srcY];

                        if (value <= 1.0)
                        {
                            field[left + x, top + y] = value;
                        }
                    }
                }
            }
    ...
    {
        Image?.Dispose();
        NativeGlitch?.Dispose();
    }
}

public static class PicturatorExporter
{
    // Converts raster data and multiplex ball schedules into osu! slider control points
    public static List<Vector2> GeneratePath(PicturatorExportRequest request, double circleSize)
    {
        ...
        if (ball != null)
        {
            ball.TemporalLength = Math.Round(request.Duration);
        }
        ...
        var (points, _) = Tools.SlideratorStuff.SliderPicturator.Picturate(
            ...
            true, true, true, true, true, true, 
            request.Quality,
            request.MotionGraph == null ? null : t => BallMotionGraph.Evaluate(request.MotionGraph, t), 
            request.NativeRadiusPixels, 
            request.CompositeField ?? request.NativeGlitch?.Build(request.NativeRadiusPixels, request.Quality), 
            request.MinimumTumourLength,
        ...
        // Round coordinates to integer boundaries expected by the osu! file format
        for (int i = 0; i < points.Count; i++)
        {
            points[i] = points[i].Rounded();
        }

        return points;
    }

    // Injects generated slider hitobject and precise tick-suppression timing points into the beatmap
    public static HitObject Export(PicturatorExportRequest request)
    {
        ...
        // 1. Generate path vertices and calculate total pixel length
        ...
        var slider = new HitObject(startTime, 0, SampleSet.None, SampleSet.None)
        {
            ...
            NewCombo = request.Shape?.NewCombo ?? false,
            ComboSkip = request.Shape?.ComboSkip ?? 0
        };
        slider.SetAllCurvePoints(points);

        // 3. Inject timing points to match target duration and suppress slider ticks
        ...
        // Place inherited timing points 1 ms earlier to enforce velocity scaling
        ...
        // Clean up conflicting timing points at insertion timestamps
        foreach (var tp in timing.TimingPoints.Where(tp => tp.Offset == slider.Time || tp.Offset == startTime).ToList())
        {
            timing.Remove(tp);
        }
        ...
        // 4. Update beatmap hitobjects and colors
        ...
        // 5. Create backup and commit changes to disk
